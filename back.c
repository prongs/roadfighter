...
{
	 dir= rand()%12-5;
    LoadGLTextures("Data/lesson6/background.bmp",&background);				// Load The Texture(s) 
    LoadGLTextures("Data/lesson6/car3.bmp",&cars[0].photo);
    LoadGLTextures("Data/lesson6/car_burning.bmp",&cars[1].photo);
    LoadGLTextures("Data/lesson6/pagal.bmp",&cars[2].photo);			
    LoadGLTextures("Data/lesson6/ullu.bmp",&cars[3].photo);
	LoadGLTextures("Data/lesson6/abc.bmp",&super);
    LoadGLTextures("Data/lesson6/end.bmp",&finish[0].photo);
	cars[0].prop1 = 0;
	cars[0].prop2 = 0;
	cars[1].prop1 = 0;
	cars[1].prop2 = 0;
...
//Now we load the road.
   ...
   // glTexCoord2f(1.0f, len); glVertex3f( 1.5f+len*shift/length, 1.5f-3.0f*part+3.0*len,  1.0f);	// Top Right Of The Texture and Quad
    ...
    if(!burning)   
        ...
        glBindTexture(GL_TEXTURE_2D,cars[1].photo);
	...
	visible[filled].x = (rand()%(2*695))/1000.0-0.695;
    ...
    if(distance>=100)
{
	...
	glBegin(GL_QUADS);
		finish[0].x=0.0f;
		finish[0].y-=velocity;
	glTexCoord2f(0.0f, 0.0f); glVertex3f(finish[0].x-0.695,finish[0].y-0.05, 1.1f);	// Bottom Left Of The Texture and Quad
	glTexCoord2f(1.0f, 0.0f); glVertex3f( finish[0].x+0.695,finish[0].y-0.05, 1.1f);	// Bottom Right Of The Texture and Quad
	glTexCoord2f(1.0f, 1.0f); glVertex3f( finish[0].x+0.695,finish[0].y+0.05, 1.1f);	// Top Right Of The Texture and Quad
	glTexCoord2f(0.0f, 1.0f); glVertex3f(finish[0].x-0.695,finish[0].y+0.05, 1.1f);	// Top Left Of The Texture and Quad    
	glEnd();
